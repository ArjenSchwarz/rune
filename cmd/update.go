...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	if updateBlockedBy != "" {
		newBlockedBy = parseRequirementIDs(updateBlockedBy)
		if len(newBlockedBy) == 0 {
			return fmt.Errorf("invalid --blocked-by value %q: no task IDs found", updateBlockedBy)
		}
	}
