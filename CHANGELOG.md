...
- **Input Validation**: `rune add` (and `add --phase`, `add` with stream/owner/blocked-by options, `update --title`, and the batch JSON `add`/`add-phase` operations) now rejects an empty or whitespace-only task title instead of writing an unparseable bullet (e.g. `- [ ] 1. `) that a subsequent `list` or `find` then fails to read back. The check lives in the shared `validateTaskInput` helper used by every title-setting path, matching the equivalent fix already applied to `create` titles
